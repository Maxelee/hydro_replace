...
class HydroReplacePipeline:
    ...
    def load_halo_matches(self, snap_num: int) -> 'MatchedCatalog':
        """
        Load pre-computed halo matches for this snapshot, or compute on the fly.
        
        Looks for matches file at standard location. If not found, computes
        matches using BijectiveMatcher and saves for future use.
        """
        from hydro_replace.data.bijective_matching import BijectiveMatcher
        
        # Try to load pre-computed matches for this resolution
        match_dir = Path(self.config.output_dir) / 'matches'
        match_file = match_dir / f'matched_snap{snap_num:03d}.h5'
        
        if match_file.exists():
            logger.info(f"    Loading pre-computed matches from {match_file}")
            return MatchedCatalog.load_hdf5(match_file)
        
        # Compute matches on the fly using BijectiveMatcher
        logger.info(f"    Computing halo matches on the fly...")
        logger.info(f"    DMO: {self.config.dmo_basepath}")
        logger.info(f"    Hydro: {self.config.hydro_basepath}")
        
        matcher = BijectiveMatcher(
            dmo_basePath=self.config.dmo_basepath,
            hydro_basePath=self.config.hydro_basepath,
            snapNum=snap_num,
            n_bound_particles=50,  # Lower for faster matching
            min_overlap_fraction=0.5,
            min_halo_mass=self.config.mass_min,
            min_particles=100,  # Minimum particles per halo
        )
        ...
        logger.info(f"    Found {matched.n_matches} matched halo pairs")
        
        # Save matches for future use
        match_dir.mkdir(parents=True, exist_ok=True)
        matched.save_hdf5(match_file)
        logger.info(f"    Saved matches to {match_file}")
    ...
    def apply_bcm(self, dmo_particles: Dict, matched_halos: 'MatchedCatalog') -> Dict:
        ...
        # BCM model parameters - full defaults from BaryonForge examples
        # These are reasonable defaults from BaryonForge
        if self.config.bcm_model == 'Schneider19':
            # Full params from Schneider19 example notebook 08_Reproduce_Schneider19.ipynb
            h = 0.6774
            bpar = dict(
                theta_ej=4, theta_co=0.1,
                mu_beta=1, M_c=1e14/h,
            ...
            # Full params from example notebook 15_Plot_Profiles_Schneider25.ipynb
            bpar = dict(
                epsilon0=4, epsilon1=0.5, alpha_excl=0.4, p=0.3, q=0.707, M_c=1e15, mu=0.8,
            ...
            # Full params from Arico20 example notebook 11_Plot_Profiles_Arico.ipynb
            h = 0.6774
            bpar = dict(
                cdelta=4,
                alpha_g=2, epsilon_h=0.015, M1_0=2.2e11/h,
                alpha_fsat=1, M1_fsat=1, delta_fsat=1, gamma_fsat=1, eps_fsat=1,
                M_c=1.2e14/h, eta=0.6, mu=0.31, beta=0.6, epsilon_hydro=2.236,  # sqrt(5)
                M_inn=3.3e13/h,
