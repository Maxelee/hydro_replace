#!/bin/bash
#SBATCH -J bcm_2500
#SBATCH -o logs/bcm_2500_%A_%a.o
#SBATCH -e logs/bcm_2500_%A_%a.e
#SBATCH -N 8
...
# Array job for BCM pipeline - L205n2500TNG
#
...
#
# Memory requirements:
#   - BCM only loads DMO particles (~250 GB total)
#   - 16 tasks × ~16 GB/task = safe with ~90 GB available
#
# Parameters:
...
#
# Usage:
#   sbatch batch/run_bcm_2500_array.sh
#
# After completion, run lens planes:
#   sbatch --dependency=afterok:<JOBID> batch/run_bcm_2500_lensplanes.sh
...
for BCM_MODEL in "${BCM_MODELS[@]}"; do
    ...
    ls -lh ${OUTPUT_DIR}/snap$(printf "%03d" ${SNAP})/projected/bcm_${BCM_MODEL,,}_M${MASS_MIN/./p}.npz 2>/dev/null || echo "    (no maps)"
done
