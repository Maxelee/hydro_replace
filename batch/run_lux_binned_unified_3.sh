...
MODEL_IDX=$(((SLURM_ARRAY_TASK_ID+2000) / N_REALIZATIONS))
REALIZATION=$(((SLURM_ARRAY_TASK_ID+2000) % N_REALIZATIONS))

if [ $MODEL_IDX -ge $N_MODELS ]; then
    echo "Task ID $((SLURM_ARRAY_TASK_ID+2000)) exceeds model count ($N_MODELS), exiting"
    exit 0
fi
...
echo "=============================================="
echo "Lux Pipeline - Unified (Array Task $((SLURM_ARRAY_TASK_ID+2000)) / 2039)"
