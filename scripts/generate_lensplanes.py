...
# BCM Parameters (from parameters_best.md)
BCM_PARAMS = {
    'Arico20': dict(
...
def process_snapshot(args, snap_idx, snap_info, random_state, bcm_models=None):
    """
    Process a single snapshot to generate lens planes.
    ...
    random_state : RandomizationState
    ...
    grid_res = args.grid_res  # Use command-line argument
    pps = CONFIG['planes_per_snapshot']
    scale_factor = 1.0 / (1.0 + redshift)
    
    # Get randomization for this snapshot
    ...
    if args.model in ['hydro', 'replace', 'all']:
        ...
        hydro_coords = np.zeros((0, 3), dtype=np.float32)
        hydro_masses = np.zeros(0, dtype=np.float32)
    
    # Apply randomization (translation + flip)
    dmo_coords = apply_randomization(dmo_coords, rand_params, box_size)
    if len(hydro_coords) > 0:
        hydro_coords = apply_randomization(hydro_coords, rand_params, box_size)
    
    if rank == 0:
        print(f"  Rank 0: {len(dmo_coords):,} DMO, {len(hydro_coords):,} hydro particles")
    
    # ========================================================================
    # Load halo catalog (for replace and BCM)
    # ========================================================================
    halo_dmo = None
    if args.model in ['replace', 'bcm', 'all'] or (bcm_models is not None):
        ...
        # Get halo properties (transform positions too!)
        halo_positions = halo_dmo['GroupPos'][halo_indices] / 1e3
        halo_positions = apply_randomization(halo_positions, rand_params, box_size)
        halo_radii = halo_dmo['Group_R_Crit200'][halo_indices] / 1e3
        halo_masses = halo_dmo['Group_M_Crit200'][halo_indices] * CONFIG['mass_unit']
    ...
    # ========================================================================
    if rank == 0:
        print("\n[3/4] Generating lens planes...")
    
    # Compute mean surface density for normalization
    # Σ_mean = Ω_m × ρ_crit × L_box / pps
    slice_thickness = box_size / pps
    ...
    for plane_idx in range(pps):
        ...
        if rank == 0:
            print(f"\n  Plane {plane_num} (slice {plane_idx+1}/{pps})...")
        
        # Generate fields for requested models
        models_to_process = []
        if args.model == 'all':
        ...
        for model_name in models_to_process:
            # Build output directory name (include mass cut for replace)
            ...
            output_dir = os.path.join(
                args.output_dir, f'L205n{args.sim_res}TNG', dir_name
            )
            if rank == 0:
            ...
            if args.skip_existing and os.path.exists(output_file):
                if rank == 0:
                    print(f"    Skipping {model_name} (exists)")
                continue
            ...
            if rank == 0:
                ...
                if nan_count > 0:
                    print(f"    WARNING: {nan_count} NaN values found, setting to 0")
                    delta_dz = np.nan_to_num(delta_dz, nan=0.0)
                
                write_density_plane(output_file, delta_dz, grid_res)
                print(f"    Wrote {model_name}: {output_file}")
    
    # ========================================================================
    # Write config file (once per model directory)
    # ========================================================================
    if rank == 0 and snap_idx == len(SNAPSHOT_CONFIG) - 1:
        print("\n[4/4] Writing config files...")
        ...
        for model_name in models_to_process:
            output_dir = os.path.join(
                args.output_dir, f'L205n{args.sim_res}TNG', model_name
            )
...
def main():
    ...
    parser.add_argument('--seed', type=int, default=2020,
                       help='Random seed for consistent randomization')
    ...
    random_state = RandomizationState(seed=args.seed)
    ...
    for snap_idx in snap_indices:
        ...
        process_snapshot(args, snap_idx, snap_info, random_state, bcm_models)
